...
vae_mod <- nn_module("CVAE",
                 initialize = function(input_dim, latent_dim, breadth) {
                   ...
                   self$encoder <- nndag(m_1 = ~ input_dim,
                                           ...
                                           m_2 = m_1 ~ breadth,
                                           x_2 = x_1 ~ breadth,
                                           me_2 = me_1 + m_2 + x_2 ~ breadth,
                                           m_3 = m_2 ~ breadth,
                                           x_3 = x_2 ~ breadth,
                                           me_3 = me_2 + m_3 + x_3 ~ breadth,
                                           m_means = me_3 ~ latent_dim,
                                           m_logvars = me_3 ~ latent_dim,
                                           .act = list(nn_relu,
                                                       m_logvars = nn_identity,
                                                       m_means = nn_identity))
                   self$decoder <- nndag(mz_1 = ~ input_dim,
                                         z_1 = ~ latent_dim,
                                         md_1 = mz_1 + z_1 ~ breadth,
                                         mz_2 = mz_1 ~ breadth,
                                         z_2 = z_1 ~ breadth,
                                         md_2 = md_1 + mz_2 + z_2 ~ breadth,
                                         mz_3 = mz_2 ~ breadth,
                                         z_3 = z_2 ~ breadth,
                                         md_3 = md_2 + mz_3 + z_3 ~ breadth,
                                         out = md_3 ~ input_dim,
                                         .act = list(nn_relu,
                                                     out = nn_sigmoid))
                 ...
                 loss_function = function(reconstruction, input, missing, mean, log_var, loggamma) {
                   ...
                   recon1 <- (torch_sum(nnf_binary_cross_entropy(reconstruction, input, reduction = "none") * missing, dim = 2L) / torch_exp(loggamma)) / (torch_sum(missing, dim = 2L) / self$input_dim)
                   loss <- torch_mean(recon1 + kl)
                 ...
                 forward = function(x, m) {
                   ...
                   recon <- self$decoder(mz_1 = m, z_1 = z)
                   list(recon, x, m, means, log_vars)
                 })
...
for (epoch in 1:num_epochs) {
    ...
    coro::loop(for (b in train_dl) {
        ...
        batchnum <- batchnum + 1
        optimizer$zero_grad()
        c(reconstruction, input, missing, mean, log_var) %<-% vae(b$gb$cuda(), b$ob$cuda())
        c(loss, reconstruction_loss, kl_loss) %<-%
        ...
        train_acc <- 1 - (torch_sum(torch_abs(torch_round(reconstruction) - input) * missing) / torch_sum(missing))
        ...
        if((epoch %% 100 == 0 || epoch == 1) && batchnum == 1) {
          ...
          with_no_grad({
            ...
            c(reconstruction, input, missing, mean, log_var) %<-% vae(val_dat$gb, val_dat$ob)
            c(val_loss, val_reconstruction_loss, val_kl_loss) %<-%
              vae$loss_function(reconstruction, input,
                                missing,
                                mean, log_var,
                                torch_tensor(loggamma, device = "cuda")
          ...
          vae$train()
          val_acc <- 1 - (torch_sum(torch_abs(torch_round(reconstruction) - input) * missing) / torch_sum(missing))
        }
...
torch_save(vae, "data/grambank_vae_interpolate.to")

vae <- torch_load("data/grambank_vae_interpolate.to")

test_dat <- test_dl$.iter()$.next()
test_dat$ob <- test_dat$ob$cuda()
test_dat$gb <- test_dat$gb$cuda()
...
with_no_grad({
    ...
    #val_obs <- torch_ones_like(val_dat$gb)
    c(reconstruction, input, missing, mean, log_var) %<-% vae(test_dat$gb, test_dat$ob)
    c(test_loss, test_reconstruction_loss, test_kl_loss) %<-%
      vae$loss_function(reconstruction, input,
                        missing,
                        mean, log_var,
                        torch_tensor(loggamma, device = "cuda")
...
test_acc <- 1 - (torch_sum(torch_abs(torch_round(reconstruction) - input) * missing) / torch_sum(missing))
...
preds <- as.matrix(reconstruction$cpu()) |>
  as.data.frame() |>
  mutate(id = 1:n()) |>
  pivot_longer(-id, names_to = "feature", values_to = "prediction")

truth <- as.matrix(input$cpu()) |>
  as.data.frame() |>
  mutate(id = 1:n()) |>
  pivot_longer(-id, names_to = "feature", values_to = "truth")

miss <- as.matrix(missing$cpu()) |>
  as.data.frame() |>
  mutate(id = 1:n()) |>
  pivot_longer(-id, names_to = "feature", values_to = "missing")
...
threshs <- pred_df |>
  group_by(feature) |>
  threshold_perf(truth, prediction,
                 thresholds = seq(0, 1, 0.02),
                 metrics = metric_set(j_index, accuracy))

best_threshs <- threshs |>
  filter(.metric == "accuracy") |>
  group_by(feature) |>
  slice_max(.estimate, n = 1, with_ties = FALSE) |>
  mutate(.estimate = ifelse(.threshold == 0, 0.5, .estimate))

pred_df <- pred_df |>
  left_join(best_threshs %>% select(feature, .estimate), by = "feature") |>
  mutate(.preds = make_two_class_pred(prediction, levels(truth), .estimate[1]))

acc_by_lang <- pred_df |>
  group_by(id) |>
  accuracy(truth, .preds)
mean(acc_by_lang$.estimate)
...
accuracy(pred_df, truth, .preds)
sens(pred_df, truth, .preds)
yardstick::spec(pred_df, truth, .preds)

all_ds <- grambank_dataset(grambank_all)
all_dl <- dataloader(all_ds, length(all_ds))

all_dat <- all_dl$.iter()$.next()
all_dat$ob <- all_dat$ob$cuda()
all_dat$gb <- all_dat$gb$cuda()

with_no_grad({

    #val_obs <- torch_ones_like(val_dat$gb)
    c(reconstruction, input, missing, mean, log_var) %<-% vae(all_dat$gb, all_dat$ob)
...
accuracy(pred_df |> filter(missing == 1), truth, .preds)

new_df <- pred_df |>
  ungroup() |>
  mutate(.preds = as.numeric(as.character(.preds)),
         truth = as.numeric(as.character(truth))) |>
  mutate(value = ifelse(missing == 0, .preds, truth)) |>
  select(id, feature, value) |>
  pivot_wider(names_from = feature, values_from = value)

cnames <- grambank_all |> select(starts_with("GB")) |> colnames()
colnames(new_df) <- c("id", cnames)

new_df <- new_df |>
  select(-id) |>
  mutate(Glottocode = grambank_all$Glottocode,
         Family = grambank_all$Family) |>
  select(Glottocode, Family, everything())

write_csv(new_df, "data/grambank_VAE32d_imputed.csv")

...
####### Load Data ########################
grambank <- read_csv("data/cldf/values.csv")
...
torch_save(vae, "data/grambank_vae_stage_1.to")
...
write_csv(latent_df, "data/grambank_vae_latent_codes.csv")
latent_df_means <- as.data.frame(latent_means) %>%
  mutate(Language_ID = lang_ids) %>%
  select(Language_ID, everything())

write_csv(latent_df_means, "data/grambank_vae_latent_codes_means_only.csv")

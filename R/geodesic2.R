...
init_rates <- read_csv("data/init_rates.csv")
gb_pf <- gb_pf %>%
...
gb_dl <- dataloader(gb_ds, 150, shuffle = TRUE, drop_last = FALSE)
...
mod <- mani_evo_mod(n_rates = nrow(gb_init_rates), n_dim = 11,
                    centroids, vars,
...
for(epoch in 1:n_epoch) {
  ...
  coro::loop(for (b in gb_dl) {
      ...
      loss <- (res[[1]] + res[[3]]) * 10 #/ length(gb_dl) ## + res[[2]]
      loss$backward()
...
torch_save(mod, "data/mani_evo_mod_run_new_full_param_1.to")
...
plot(cbind(as.vector(test_zs[ , 3, ]), as.vector(test_zs[, 4, ])), type = "n")
points(as.matrix(centroids$cpu())[, 3:4], cex = 0.2, col = "green")
for(i in seq_len(dim(test_zs)[1])) {
  points(cbind(as.vector(test_zs[i, 3, ]), as.vector(test_zs[i, 4, ])), type = "l", col = "red")
  print(i)
}
points(as.matrix(centroids$cpu())[, 3:4], cex = 0.2, col = "green")
